    ...
    <div className="flex flex-col min-h-screen pb-20">
      ...
      <main className="flex-1 px-4 py-6 max-w-lg mx-auto w-full">
        <section className="mb-8 flex items-center gap-4">
          ...
          />
          <div>
            <h1 className="text-2xl font-bold text-lawbox-navy mb-1 flex items-center gap-2">
              <Scale className="w-6 h-6 text-lawbox-gold" />
              LAWBOX
            </h1>
            <p className="text-gray-500 text-sm">웹 기반 통합 문서 뷰어 & 편집기</p>
          </div>
        </section>
        ...
        <section className="mt-8 grid grid-cols-3 gap-3">
          {features.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-white rounded-xl p-3 border border-gray-100 text-center shadow-sm">
              <Icon className="w-5 h-5 text-lawbox-navy mx-auto mb-2" />
