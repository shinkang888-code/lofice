...
Copy-Item $apkSrc (Join-Path $androidDir "OneOffice.apk") -Force
Copy-Item (Join-Path $releases "android\INSTALL.txt") (Join-Path $androidDir "INSTALL.txt") -Force
$androidZip = Join-Path $releases "OneOffice-Android-0.1.0.zip"
...
Get-ChildItem $releases -Filter "*.zip" | ForEach-Object {
